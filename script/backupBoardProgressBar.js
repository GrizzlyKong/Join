...
function addTodo() {
  ...
  let taskHTML = `
    <div id="${taskId}" class="board-task-card pointer" draggable="true" onclick="openTaskInfos('${taskId}', '${title}', '${description}', '${category}', '${dueDate}', ${JSON.stringify(subtasks).split('"').join("&quot;")})">
      ...
      <div class="board-task-card-users">
        ...
        <div class="board-task-card-priority"><img id="priorities-todo-${taskId}" src=""></div>
      </div>
    </div>
  `;
...
function updateProgressBar() {
  const totalSubtasks = 2; // Maximale Anzahl der Subtasks
  const currentSubtasks = document.querySelectorAll("#added-subtasks .added-subtask").length;
  const progressPercent = (currentSubtasks / totalSubtasks) * 100;
  ...
  let subtaskText = document.querySelector(".board-task-card-subtasks-amount");
  if (subtaskText) {
    subtaskText.textContent = `${currentSubtasks}/${totalSubtasks} Subtasks`;
  }
}
...
function editExistingSubtask(index, subtask) {
  ...
  // Überprüfen, ob das Subtask-Element existiert
  if (!subtaskElement) {
  ...
  // Ersetzen Sie die bestehende Subtask durch die bearbeitbare Form
...
function correctSubtask(taskId) { // taskId hinzugefügt
  let input = document.getElementById("add-subtasks").value.trim();
  if (input !== "") {
    const currentSubtasks = document.querySelectorAll(`#${taskId} .added-subtask`).length;
    if (currentSubtasks < 2) {
      ...
      updateTaskProgress(taskId, 0, currentSubtasks + 1); // Aktualisiere Fortschritt
    } else {
